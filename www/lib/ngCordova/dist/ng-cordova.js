...
angular.module('ngCordova.plugins.device', [])
  ...
  .factory('$cordovaDevice', [function () {
    ...
    return {
      ...
      // Warning: device.name is deprecated as of version 2.3.0. Use device.modal instead.
      getName: function () {
        return device.name;
      },
  ...
  .factory('$cordovaKeychain', ['$q', function ($q) {
    ...
    return {
      getForKey: function (key, serviceName) {
        ...
        cordova.plugins.Keychain.getForKey(function (value) {
          defer.resolve(value);
        }, function (error) {
          defer.reject(error);
        }, key, serviceName);
      ...
      setForKey: function (key, serviceName, value) {
        var defer = $q.defer();
	      ...
	      cordova.plugins.Keychain.setForKey(function () {
          defer.resolve();
        }, function (error) {
          defer.reject(error);
      ...
      removeForKey: function (key, serviceName) {
        var defer = $q.defer();
	      ...
	      cordova.plugins.Keychain.removeForKey(function () {
          defer.resolve();
        }, function (error) {
          defer.reject(error);
        }, key, serviceName);
...
// install   :
// link      :
    angular.module('ngCordova.plugins.version', [])

        .factory('$cordovaVersion', ['$q', function ($q) {

            return {
                getVersion: function () {
					var q = $q.defer();
                    cordova.getAppVersion(function(result){
	                    q.resolve(result);
                    });
