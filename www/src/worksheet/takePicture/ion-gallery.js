(function () {
  ...
  function ionGallery($ionicPlatform, ionGalleryHelper, ionGalleryConfig) {
    ...
    return {
      ...
      link: link,
      replace: true,
      templateUrl: 'src/worksheet/takePicture/gallery.html'
    };
  ...
  function ionSlider($ionicModal,ionGalleryHelper,$ionicPlatform,$timeout,$ionicScrollDelegate){
    ...
    function controller($scope){
      ...
      $scope.showImage = function(index){
        ...
        /*if(previndex < 0 && nextindex < $scope.ionGalleryItems.length){  //点击第一张图片
          $scope.slides[0] = $scope.ionGalleryItems[index];
          $scope.slides[1] = $scope.ionGalleryItems[nextindex];
          if(nextindex+1 < $scope.ionGalleryItems.length){
            $scope.slides[2] = $scope.ionGalleryItems[nextindex+1];
            lastSlideIndex = 2;
          }else{
            lastSlideIndex = 1;
          }
          $scope.selectedSlide = 0;
        }else if(previndex < 0 && nextindex >= $scope.ionGalleryItems.length){  //只有一张图片
          $scope.slides[0] = $scope.ionGalleryItems[index];
          $scope.selectedSlide = 0;
          lastSlideIndex = 0;
        }else if(previndex >= 0 && nextindex >= $scope.ionGalleryItems.length){
          if(previndex-1 >= 0){
            $scope.slides[0] = $scope.ionGalleryItems[previndex-1];
            $scope.slides[1] = $scope.ionGalleryItems[previndex];
            $scope.slides[2] = $scope.ionGalleryItems[index];
    ...
    function link(scope, element, attrs) {
      ...
      scope.loadModal = function(){
        $ionicModal.fromTemplateUrl('src/worksheet/takePicture/slider.html', {
          scope: scope,
          animation: 'fade-in'
        }).then(function(modal) {
          _modal = modal;
      ...
      scope.xbrDeleteFromRemote = function(item){
        scope.closeModal();
        var xbrScope = angular.element("#worksheet-takepicture-view").scope();
