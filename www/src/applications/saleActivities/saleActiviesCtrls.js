        ...
        function ($scope, $rootScope, $state, $timeout, $ionicLoading, $ionicPopover, $ionicModal, $cordovaToast, $ionicScrollDelegate,
            ...
            $scope.changeFilterFlag = function (e) {
                ...
                if ($scope.filterFlag) {
                    ...
                    angular.element('#saleChanListFilterId').attr('ng-if', 'ng-show');
                }
                e.stopPropagation();
            };
        ...
        function ($scope, $rootScope, $state, $ionicHistory, $ionicScrollDelegate,
            ...
            $scope.submit = function () {
                switch ($scope.myProcess.code) {
                    //达成共识
                    case 'IT_SA0021':
                        $scope.makeConsensus.item.push({
                            "MODE": "I",
                            "ZZSXNR": $scope.input.progress,  //事项内容
                        ...
                        console.log($scope.makeConsensus);
                        break;
                    //跟进事项
                    case 'IT_SA0022':
                        $scope.followUpMatter.item.push({
                            "MODE": "I",
                            "ZZSXNR_1": $scope.input.progress,
                    ...
                    //政策解读
                    case 'IT_SA0023':
                        $scope.policyDecode.item.push({
                            "MODE": "I",
                            "ZZSXNR_2": $scope.input.progress,
                            "ZZZCYX": $scope.process.affect,
                            "ZZNOTE_3": ""
                        });
                        console.log($scope.policyDecode);
                        break;
                }
            ...
            //删除
            $scope.deleteProcess = function (x, arr, index) {
                if (!$scope.isEdit) {
                    return;
                }
            ...
            //替换
            $scope.reWriteProcess = function (x, type) {
                switch (type) {
                    //跟进事项
                    case 'followUpMatter':
                        break;
            ...
            //新建
            $scope.createProcess = function (x, type) {
                switch (type) {
                    //跟进事项
                    case 'followUpMatter':
                        break;
