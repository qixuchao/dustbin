...
ContactsModule
    .controller('contactQueryCtrl',['$cordovaDialogs','Prompter','$scope','$rootScope','$state','$http','HttpAppService','$timeout','$ionicPopover','$ionicActionSheet','$window','$cordovaToast','$ionicScrollDelegate','ionicMaterialInk','contactService','$ionicLoading',
                            ...
                            if (response.ET_OUT_LIST.item.length == 0) {
                                ...
                                $.each(response.ET_OUT_LIST.item, function (n, value) {
                                    if($scope.config.contactfiledvalue===""){
                                        $scope.contact_query_list=new Array;
                                        $scope.contactisshow=false;
                                        //console.log($scope.contactisshow);
                                    }else{
        ...
        //$scope.config.queryResultScrollDelegate = $ionicScrollDelegate.$getByHandle("contactListResult");
        ...
        $scope.conatct_querynumber = function(data){
            //console.log(data);
            if(data.TEL_NUMBER == undefined && data.MOB_NUMBER == undefind){
                $cordovaToast.showShortBottom('没有数据');
            }else{
                var number=[];
                if(data.TEL_NUMBER!==undefined&&data.TEL_NUMBER!==""){
                    number.push({text:data.TEL_NUMBER});
                    //console.log(data.TEL_NUMBER);
                }
                if(data.MOB_NUMBER!==undefined&&data.MOB_NUMBER!==""){
    ...
    .controller('contactDetailCtrl',['LoginService','$scope','$rootScope','$state','$ionicHistory','Prompter','HttpAppService','$cordovaToast','$ionicLoading','$ionicScrollDelegate','$ionicPopup','ionicMaterialInk','contactService','$window','$ionicActionSheet',
        ...
        $scope.$on("$stateChangeSuccess", function (event, toState, toParams, fromState, fromParam){
            ...
            if(fromState && toState && fromState.name == 'ContactEdit'){
                flag1=true;
            }
            //根据路由进行刷新
    ...
    .controller('contactCreateCtrl',['$cordovaDialogs','$scope','$rootScope','$ionicHistory','$state','Prompter','$cordovaDatePicker','customeService','LoginService','saleActService','HttpAppService','$cordovaToast','$ionicModal','$ionicLoading','$ionicScrollDelegate','$ionicPopup','ionicMaterialInk','contactService','$window','$ionicActionSheet',
        ...
        $scope.autoHeight1=function(){
            var text = document.getElementById("textarea3");
            autoTextarea1(text);
        };
        //根据用户权限显示
        $scope.showFlag=true;
    ...
    .controller('contactEditCtrl',['$scope','$rootScope','$timeout','$state','Prompter','$http','HttpAppService','$cordovaToast','$ionicLoading','$ionicScrollDelegate','$ionicPopup','$cordovaDatePicker','ionicMaterialInk','LoginService','contactService','$window','$ionicActionSheet',
        ...
        $scope.EditCancel = function(){
           Prompter.ContactCreateCancelvalue1("编辑");
            ...
            $scope.autoHeight1=function(){
                var text = document.getElementById("textarea4");
                autoTextarea1(text);
            };
            //根据用户类型
            $scope.showFlag=true;
        ...
        //选择日期
            $scope.selectCreateTime1 = function () { // type: start、end
                ...
                //$scope.contactedit.BIRTHDT="2016/1/21";
        ...
        $scope.contactKeepEditvalue = function(){
            ...
            data.IS_CUSTOMER.TITLE_MEDI=$scope.config.currentTitile;
            ...
            if(data.IS_CUSTOMER.NAME_LAST == ''|| data.IS_CUSTOMER.NAME_LAST == undefined
                || $scope.config.currentCountry == ''|| $scope.config.currentCountry == undefined){
                $cordovaToast.showShortCenter('请输入联系人姓名或国家');
        ...
        $scope.config = {
            ...
            currentLanguage:$scope.contactedit.LANGU.toString(),
            currentTitile:$scope.contactedit.TITLE
        };
            //console.log(angular.toJson($scope.config)+"sdsdsd");
            //console.log($scope.contactedit.LANGU);
        $scope.cascade=function(){
            //http://117.28.248.23:9388/test/api/CRMAPP/LIST_CITY
            var url=ROOTCONFIG.hempConfig.basePath + "LIST_CITY";
            var data={
        ...
        $scope.cascade2=function(){
            ...
            if($scope.config.currentProvence!=undefined){
                //console.log($scope.config.currentProvence+"省份标识");
