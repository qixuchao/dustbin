settingsModule.controller("forgetPassCtrl", [
	'$scope',
	'$state',
	'$ionicHistory',
	'$timeout',
	...
	function($scope, $state, $ionicHistory, $timeout, $interval, $cordovaToast, HttpAppService, SettingsService, worksheetDataService, Prompter, $rootScope){
		...
		$scope.$on("$stateChangeSuccess", function (event, toState, toParams, fromState, fromParam){
            if(toState.name == "forgetPass" || fromState.name=="forgetPass"){
            	__initConfig();
            }
            if(fromState && toState && fromState.name == 'login' && toState.name == 'forgetPass'){
		...
		$scope.confirmChangePass = function(){
			if($scope.config.username.length == 0){
				$cordovaToast.showShortBottom("请输入用户名!");
				return;
			}
